...
Outputs:
  - llm_judge_results/evaluation_log.txt
  ...
  - llm_judge_results/model_summary.txt
...
# Anthropic rejects base64 image payloads over 5 MB.
# Keep a small safety margin so the final encoded/uploaded image stays below the limit.
ANTHROPIC_IMAGE_MAX_BYTES = 5 * 1024 * 1024
IMAGE_TARGET_MAX_BYTES = ANTHROPIC_IMAGE_MAX_BYTES - 150_000
...
def image_to_anthropic_block(image_path: Path) -> Dict[str, Any]:
    """
    Convert an image to an Anthropic-compatible base64 image block.

    Anthropic rejects images above 5 MB. This function keeps the image as large/high-quality
    as possible while forcing it below the limit. It first tries the original bytes if the
    file is already small enough. If not, it recompresses as JPEG and gradually lowers quality
    and, only if necessary, scales down dimensions.
    """
    original_size = image_path.stat().st_size

    # If the original file is already safely under the limit, send it unchanged.
    if original_size <= IMAGE_TARGET_MAX_BYTES:
    ...
    # Otherwise, compress while keeping the image close to the 5 MB cap.
    ...
    while current_long_edge >= IMAGE_MIN_LONG_EDGE:
        ...
        if current_long_edge < long_edge:
            scale = current_long_edge / long_edge
            new_size = (
        ...
        # Try high quality first, then step down.
        for quality in range(IMAGE_INITIAL_QUALITY, IMAGE_MIN_QUALITY - 1, -5):
            ...
            # Track the largest valid version, so we get as close to 5 MB as possible.
            if size <= IMAGE_TARGET_MAX_BYTES and size > best_size:
                best_data = data
                best_size = size
                best_quality = quality
                best_long_edge = current_long_edge

            # Since we are trying quality from high to low, the first valid quality at this
            # dimension is likely the best at this dimension.
            if size <= IMAGE_TARGET_MAX_BYTES:
                break

        if best_data is not None:
            break

        # Only reduce dimensions if quality reduction was not enough.
        current_long_edge = int(current_long_edge * 0.9)

    if best_data is None:
        # Last-resort fallback: keep shrinking until valid.
    ...
    if best_data is None:
        raise RuntimeError(f"Could not compress image below Anthropic limit: {image_path}")

    log(
        f"Compressed image for Anthropic: {image_path.name} | "
        f"original={original_size:,} bytes | sent={best_size:,} bytes | "
        f"quality={best_quality} | long_edge={best_long_edge}"
    )

    encoded = base64.b64encode(best_data).decode("utf-8")
...
def main() -> None:
    ...
    log(f"Evaluating {len(metadata)} images x {len(MODEL_RUNNERS)} models = {total} image-poem pairs")
