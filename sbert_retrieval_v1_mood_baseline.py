# V1: SBERT retrieval with baseline normalization using 17 mood prompts. Partially
# fixed centroid issue — removed the worst offender but poems like "Technical Notes"
# and "To Wordsworth" still float to the top across unrelated queries. Baseline
# query set too narrow. Kept for documentation.
...
SBERT_BASELINE_FILE = os.path.join(CACHE_DIR, "sbert_poem_baseline_v1.npy")
...
def build_mood_baseline():
    ...
    prompt_embs = sbert.encode(mood_prompts, convert_to_numpy=True, show_progress_bar=False)
    norms = np.linalg.norm(prompt_embs, axis=1, keepdims=True)
    prompt_embs = prompt_embs / np.where(norms == 0, 1, norms)  # (17, 384)

    baselines = []
    for start, end in chunk_map:
        chunk_embs = all_chunk_embs[start:end]
        sims = prompt_embs @ chunk_embs.T   # (17, n_chunks)
        max_per_prompt = np.max(sims, axis=1)  # (17,)
        baselines.append(float(np.mean(max_per_prompt)))
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("SBERT Retrieval V1: Mood Prompt Baseline")
    print("=" * 60)
